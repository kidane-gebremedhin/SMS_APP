...
<hr>
	...
	<a href="{{route('rounds.edit', $round->id)}}" value="{{$round->id}}" class="get btn btn-sm btn-primary" nextUrl="{{route('rounds.edit', $round->id)}}"><i class="fa fa-edit"></i> 
		{{App\Global_var::getLangString('Edit', $language_strings)}}
	</a>
	<a href="{{route('rounds.delete', $round->id)}}" value="{{$round->id}}" class="get btn btn-sm btn-danger" nextUrl="{{route('rounds.delete', $round->id)}}"><i class="fa fa-trash"></i> 
