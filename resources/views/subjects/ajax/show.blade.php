...
<hr>
	...
	<a href="{{route('subjects.edit', $subject->id)}}" value="{{$subject->id}}" class="get btn btn-sm btn-primary" nextUrl="{{route('subjects.edit', $subject->id)}}"><i class="fa fa-edit"></i> 
		{{App\Global_var::getLangString('Edit', $language_strings)}}
	</a>
	<a href="{{route('subjects.delete', $subject->id)}}" value="{{$subject->id}}" class="get btn btn-sm btn-danger" nextUrl="{{route('subjects.delete', $subject->id)}}"><i class="fa fa-trash"></i> 
