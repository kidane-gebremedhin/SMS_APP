<div class="col-md-12">
	<div class="col-md-10">
		<h4 class="text-danger">
			...
			<a href="{{route('wereda_teacher_acceptance_capacities.index')}}" class="get btn btn-default btn-sm" nextUrl="{{route('wereda_teacher_acceptance_capacities.destroy', $wereda_teacher_acceptance_capacity->id)}}"> 
				{{App\Global_var::getLangString('No', $language_strings)}}
			</a>
			@endif
			@if($currentUser->isGranted('wereda_teacher_acceptance_capacities.delete'))
			<a href="{{route('wereda_teacher_acceptance_capacities.destroy', $wereda_teacher_acceptance_capacity->id)}}" value="{{$wereda_teacher_acceptance_capacity->id}}" class="get btn btn-danger btn-sm" nextUrl="{{route('wereda_teacher_acceptance_capacities.index')}}"><i class="fa fa-trash"></i> 
