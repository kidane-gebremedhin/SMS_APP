  ...
  <div class="col-md-12">
    <div class="panel panel-success">
      <div class="panel-heading">
        <h4><label class="badge bg-green">{{$transfer_requests_from_wereda_to_weredas!=null? $transfer_requests_from_wereda_to_weredas->count(): 0}}</label>
        {{App\Global_var::getLangString('Transfer_results_from_wereda_to_weredas', $language_strings)}}
      ...
      <table class="allResultAtTimeDiv table table-striped table-hover" style="display: none;">
        ...
        <tbody>
          <?php $count=1; ?>
          @foreach($transfer_requests_from_wereda_to_weredas as $result)
          <?php 
          ...
          <tr>
            ...
            <td>{{$result->request->fromWereda!=null? $result->request->fromWereda->name:''}}</td>
            <td>{{$result->request->toWereda!=null? $result->request->toWereda->name:''}}</td>
          </tr>
          @endforeach
        </tbody>
      </table>
...
var lengthOfSingleResultInMilliSeconds={!!$setting->lengthOfSingleResultInMilliSeconds!!};
    ...
    interval=setInterval(function(){
      ...
      $(".resultsBtn").css('color', 'green');
       ...
       $('.fromDiv').empty().append(results[index].from_wereda.name);
       $('.toDiv').empty().append(results[index].to_wereda.name);
