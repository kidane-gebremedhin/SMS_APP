<div class="col-md-12">
	...
	<div class="col-md-8">
		...
		<h4 class="text-danger">
			...
			<a href="{{route('recipient_categories.index')}}" class="get btn btn-default btn-sm" nextUrl="{{route('recipient_categories.destroy', $recipient_category->id)}}"> 

				{{App\Global_var::getLangString('No', $language_strings)}}

			</a>

			<a href="{{route('recipient_categories.destroy', $recipient_category->id)}}" value="{{$recipient_category->id}}" class="get btn btn-danger btn-sm" nextUrl="{{route('recipient_categories.index')}}"><i class="fa fa-trash"></i> 
